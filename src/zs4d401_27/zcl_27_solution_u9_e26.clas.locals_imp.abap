...
"! {@link .lcl_passenger_flight} and {@link .lcl_cargo_flight}.<br/>
...
CLASS lcl_passenger_flight DEFINITION INHERITING FROM lcl_flight.
  ...
  PUBLIC SECTION.
    ...
    TYPES tt_flights TYPE STANDARD TABLE OF REF TO lcl_passenger_flight
                            WITH DEFAULT KEY.
...
*            COMPARING carrier_id connection_id flight_date.
    ...
    r_result = VALUE #( FOR <flight> IN flights_buffer USING KEY sk_carrier
                        WHERE ( carrier_id = i_carrier_id )
...
*    ENDLOOP.
  ...
  METHOD constructor.
    ...
    IF flight_raw IS NOT INITIAL.
      me->carrier_id    = EXACT #( i_carrier_id ).
      me->connection_id = i_connection_id.
      me->flight_date   = i_flight_date.
...
"! Flight Carrier - A factory logic
"! ensures that there is only one
"! instance for the same carrier ID.
CLASS lcl_carrier DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! @raising zcx_27_failed | Instantiation failed - evaluate the exception text for details.
...
*        cx_abap_auth_check_exception
    ...
    "! @parameter e_days_later | Number of days after the requested date METHODS find_passenger_flight
      ...
      CATCH cx_sy_itab_line_not_found.
        SELECT SINGLE
            ...
            FIELDS concat_with_space( carrier_id, name, 1 ) AS name,
                    currency_code
             WHERE carrier_id = @i_carrier_id
             INTO @DATA(details). "( @me->name, @me->currency_code ).
...
*              value = CONV #( i_carrier_id ).
            RAISE exception type zcx_27_failed
              EXPORTING
                textid     = zcx_27_failed=>carrier_not_exist
*                previous   =
                carrier_id = i_carrier_id.
        ...
        AUTHORITY-CHECK
        OBJECT '/lrn/carr' ID '/lrn/carr' FIELD i_carrier_id ID 'actvt' FIELD '03'.
        IF sy-subrc <> 0.
...
*                       cx_abap_auth_check_exception=>missing_authorization.
        raise EXCEPTION type zcx_27_failed
          EXPORTING
            textid     = zcx_27_failed=>carrier_no_read_auth
*            previous   =
            carrier_id = i_carrier_id.
...
*     INTO ( @me->name, @me->currency_code ).
    ...
    LOOP AT passenger_flights INTO DATA(pass_flight).
      APPEND pass_flight TO flights.
    ENDLOOP.
    ...
    LOOP AT cargo_flights INTO DATA(car_flight).
      APPEND car_flight TO flights.
    ENDLOOP.
...
*    SELECT FROM /lrn/passflight
*    FIELDS avg( seats_max - seats_occupied ) AS avg
*    WHERE carrier_id = @carrier_id
*    INTO @DATA(avg_).
*
*    r_result = avg_.
